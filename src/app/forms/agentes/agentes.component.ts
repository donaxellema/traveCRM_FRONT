...
export class AgentesComponent implements OnInit {
  ...
  agenteDataCambiante = new BehaviorSubject<any[]>([]);
  ...
  getAgentes(){
    ...
    this._agentesService.getAgentesY_Personas(obj).subscribe(
      (response:any) => {
        this.totalElementos = response.totalItems
        this.agenteDataCambiante.next(response.data)
  ...
  getAgentesSearch(){
    ...
    this._agentesService.getAgentesYPersonasSearch(obj).subscribe(
      (response:any) => {
        this.totalElementos = response.totalItems
        this.agenteDataCambiante.next(response.data)
      },
  ...
  getAgenteBY_ID(usu_id:any){
    ...
    this._agentesService.getAgentesYPersonasBY_ID(obj).subscribe(
      (response:any) => {
  ...
  dialogoEditarAGENTE_Persona(data:any, persona:any):void{
    ...
    const dialogRef = this.dialog.open(AgentesdialogComponent, {
      width: '850px',
      height: '800px',
  ...
  dialogoPostPersonas():void{
    ...
    dialogoEliminarPersona(pers_id:any,usu_id:any):void{
      ...
      Swal.fire({
        ...
        this._agentesService.postDELETEAgentes(obj).subscribe(
          (response:any) => {
            console.log(response)
            if(response.code==200){
